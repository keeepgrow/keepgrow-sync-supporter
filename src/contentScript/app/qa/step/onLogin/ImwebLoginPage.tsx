...
const ImwebLoginPage = () => {
  ...
  const onClickSignup = () => {
    const signupBtn = document.querySelector(".utilMenu a.right") as HTMLElement;
    if (!signupBtn) {
  ...
  const checkDeliveryForm = () => {
    const form = document.querySelector("#orderLogin_id .inputBox") as HTMLElement;
    if (!form) {
    ...
    const result = inputs.some((input) => {
      ...
      if (input.placeholder.includes("주문번호") || input.placeholder.includes("연락처")) {
        return true;
      }
      return false;
    });
